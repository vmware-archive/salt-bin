...
curl -LO  https://www.openssl.org/source/openssl-1.1.1d.tar.gz || curl -k -LO https://www.openssl.org/source/openssl-1.1.1d.tar.gz
...
if [ "${EXT_UID}" != "" -a "${EXT_GID}" != "" ]; then
    chown -R ${EXT_UID}:${EXT_GID} /opt
fi
